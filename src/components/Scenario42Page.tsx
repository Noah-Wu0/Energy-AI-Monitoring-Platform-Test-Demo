...
import {
  reportTemplates, generatedReports, dataSnapshots,
  getReportsForTemplate, getSnapshotsForReport, getLifecycleSteps,
  type ReportTemplate, type GeneratedReport, type DataSnapshot,
  type ReportLifecycleStep, type ReportStatus,
} from "../data/scenario42Data";

const templateIcons: Record<ReportTemplate["type"], typeof FileText> = {
  daily: FileText, weekly: FileClock, monthly: FileCheck, emergency: Siren, minister: BookOpen,
};

const STATUS_KEYS: Record<ReportStatus, string> = {
  draft: "s42.statusDraft",
  ai_generated: "s42.statusAiGenerated",
  edited: "s42.statusEdited",
  approved: "s42.statusApproved",
  published: "s42.statusPublished",
};
...
function parseAiMarkers(content: string): { text: string; isAi: boolean }[] {
  ...
  while (remaining.length > 0) {
    if (currentIsAi) {
      ...
      if (idx === -1) { parts.push({ text: remaining, isAi: true }); break; }
      parts.push({ text: remaining.substring(0, idx), isAi: true });
      remaining = remaining.substring(idx + "[/AI_GENERATED]".length);
      currentIsAi = false;
    } else {
      const idx = remaining.indexOf("[AI_GENERATED]");
      if (idx === -1) { parts.push({ text: remaining, isAi: false }); break; }
      if (idx > 0) parts.push({ text: remaining.substring(0, idx), isAi: false });
      remaining = remaining.substring(idx + "[AI_GENERATED]".length);
      currentIsAi = true;
    }
  }
...
function hasMarkdownTable(lines: string[]): boolean {
  for (let i = 0; i < lines.length - 1; i++) {
    if (lines[i].startsWith("|") && lines[i + 1].includes("---")) return true;
  }
  return false;
}
...
function renderLinesAsElements(lines: string[]): React.ReactNode[] {
  if (hasMarkdownTable(lines)) {
    ...
    while (i < lines.length) {
      ...
      if (lines[i].startsWith("### ")) elements.push(<h3 key={`ln-${i}`}>{lines[i].slice(4)}</h3>);
      else if (lines[i].startsWith("## ")) elements.push(<h2 key={`ln-${i}`}>{lines[i].slice(3)}</h2>);
      else if (lines[i].startsWith("# ")) elements.push(<h1 key={`ln-${i}`}>{lines[i].slice(2)}</h1>);
      else if (lines[i].match(/^\d+\.\s/)) elements.push(<div key={`ln-${i}`} className="s42-report-li">{lines[i]}</div>);
      else if (lines[i].trim() === "") elements.push(<div key={`ln-${i}`} className="s42-report-spacer" />);
      else elements.push(<p key={`ln-${i}`}>{lines[i]}</p>);
      i++;
    }
    return elements;
  }
  return lines.map((line, li) => {
    if (line.startsWith("### ")) return <h3 key={`ln-${li}`}>{line.slice(4)}</h3>;
    if (line.startsWith("## ")) return <h2 key={`ln-${li}`}>{line.slice(3)}</h2>;
    if (line.startsWith("# ")) return <h1 key={`ln-${li}`}>{line.slice(2)}</h1>;
    if (line.match(/^\d+\.\s/)) return <div key={`ln-${li}`} className="s42-report-li">{line}</div>;
    if (line.trim() === "") return <div key={`ln-${li}`} className="s42-report-spacer" />;
  ...
  return (
    <div className="s42-report-content">
      {parts.map((part, pi) => {
        ...
        if (part.isAi) return <div key={pi} className="s42-ai-block">{rendered}</div>;
...
export function Scenario42Page() {
  ...
  const selectedTemplate = reportTemplates.find((tp) => tp.id === selectedTemplateId) ?? reportTemplates[0];
  ...
  const handleTemplateSelect = (templateId: string) => {
    ...
    if (reports.length > 0) {
      ...
      if (snapshots.length > 0) setSelectedSnapshotId(snapshots[0].id);
    }
  };

  const statusText = t(STATUS_KEYS[selectedReport.status]);
  ...
  return (
    <div className="app-shell s42-shell">
      <header className="app-header floating-panel">
        ...
        <div className="header-actions">
          <button className="ghost-button" type="button" onClick={() => setLang(lang === "zh" ? "en" : "zh")} style={{ gap: 6 }}>
            <Globe size={15} />{t("app.lang")}
          </button>
          <button className="ghost-button" type="button"><Search size={16} />{t("app.search")}</button>
          <button className="primary-button" type="button"><FileClock size={17} />{t("app.report")}</button>
        </div>
      </header>

      <aside className="s42-left-panel">
        <div className="s42-panel-header">
          <h2><Layers size={16} />{t("s42.templatePanelTitle")}</h2>
          <span className="s42-badge-count">{t("s42.templateCount").replace("{count}", String(reportTemplates.length))}</span>
        </div>
      ...
      <main className="s42-center-panel">
        {selectedReport ? (
          <>
            <div className="s42-report-toolbar">
              ...
              <div className="s42-report-actions">
                <button className="s42-action-btn" type="button"><Eye size={14} />{t("s42.btnPreview")}</button>
                <button className="s42-action-btn" type="button"><Edit3 size={14} />{t("s42.btnEdit")}</button>
                <button className="s42-action-btn" type="button"><Printer size={14} />{t("s42.btnPrint")}</button>
                <button className="s42-action-btn primary-action" type="button"><Download size={14} />{t("s42.btnDownload")}</button>
              </div>
            </div>
            <div className="s42-report-scroll"><ReportContentRenderer content={selectedReport.content} /></div>
            {templateReports.length > 1 && (
              <div className="s42-report-selector">
                {templateReports.map((r) => (
                  <button key={r.id} className={`s42-report-tab ${selectedReportId === r.id ? "active" : ""}`} onClick={() => setSelectedReportId(r.id)} type="button">
          ...
          <div className="s42-empty-state">
            <FileText size={48} />
            <strong>{t("s42.emptyTitle")}</strong>
            <p>{t("s42.emptyDesc")}</p>
          </div>
        )}
      </main>
      ...
      <aside className="s42-right-panel">
        <div className="s42-panel-header">
          <h2><Database size={16} />{t("s42.snapshotPanelTitle")}</h2>
          <span className="s42-badge-count">{t("s42.snapshotCount").replace("{count}", String(reportSnapshots.length))}</span>
        </div>
        <div className="s42-panel-scroll">
          ...
          {selectedSnapshot && (
            <>
              <div className="s42-info-card s42-card-ai">
                <div className="s42-info-card-header"><Bot size={14} />{t("s42.aiDeclarationTitle")}</div>
                <p>{selectedReport.aiDeclaration}</p>
              </div>
              <div className="s42-info-card s42-card-approval">
                <div className="s42-info-card-header"><User size={14} />{t("s42.humanApprovalTitle")}</div>
                <p>{selectedReport.humanApproval}</p>
              </div>
              <div className="s42-info-card">
                <div className="s42-info-card-header"><ShieldCheck size={14} />{t("s42.dataProvenanceTitle")}</div>
                <p>{selectedSnapshot.aiDisclaimer}</p>
              </div>
            </>
          )}
          <div className="s42-snapshot-card">
            <div className="s42-snapshot-header"><TrendingUp size={14} /><strong>{t("s42.dataOverviewTitle")}</strong></div>
            <div className="s42-metrics-grid">
              <div className="s42-metric-item">
                <label>{t("s42.overviewLinkedReport")}</label>
                <strong>{selectedReport.title.split("—")[0].trim()}</strong>
              </div>
              <div className="s42-metric-item">
                <label>{t("s42.overviewSnapshotCount")}</label>
                <strong>{reportSnapshots.length} versions</strong>
              </div>
              <div className="s42-metric-item">
                <label>{t("s42.overviewReportStatus")}</label>
                <strong style={{ color: selectedReport.status === "published" ? "var(--status-normal)" : "var(--color-primary)" }}>{statusText}</strong>
              </div>
              <div className="s42-metric-item">
